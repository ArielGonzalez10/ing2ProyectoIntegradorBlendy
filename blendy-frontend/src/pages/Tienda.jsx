...
const Tienda = () => {
  ...
  const handleAccionBoton = (producto) => {
    ...
    if (producto.stock <= producto.stockMin) {
      alert(
        `Stock insuficiente: "${producto.descripcion}" ha alcanzado su límite mínimo.`
      );
      return;
    }
    ...
    if (cantidadEnCarrito + 1 > producto.stock) {
      alert(
        `No puedes agregar más unidades. Solo hay ${producto.stock} disponibles en total.`
      );
      return;
    }
        ...
        <aside className={`tienda-sidebar ${menuAbierto ? "abierto" : ""}`}>
          <div className="filtro-seccion">
            ...
            <ul className="filtro-lista">
              {["Todas", "Vinos", "Licores", "Refrescos", "Energeticos"].map(
                (cat) => (
                  <li
                    key={cat}
                ...
                <div key={prod.idProducto} className="tienda-producto-card">
                    ...
                    <img
                      src={
                        prod.imagenes && prod.imagenes.length > 0
                          ? `data:image/jpeg;base64,${prod.imagenes[0]}` // 🟢 Sacamos el '.descripcion'
                  ...
                  <div className="tienda-producto-info">
                    ...
                    {user?.idRol === 1 ? (
                      ...
                      <button
                        className={`btn-blendy btn-pill w-100 ${
                          sinStockDisponible ? "btn-agotado" : "btn-secundario"
                        }`}
